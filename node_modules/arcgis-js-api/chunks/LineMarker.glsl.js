...
}`),L?(x.attributes.add(b.VertexAttribute.NORMAL,"vec3"),v.addViewNormal(C),C.constants.add("tiltThreshold","float",.7),C.code.add(f.glsl`vec3 perpendicular(vec3 v) {
...
return vec2(v.y, -v.x);
      ...
      vecN displace(vecN pos, vecN prev, float displacementLen) {
        ...
        ${e.anchor===P.LineMarkerAnchor.Tip?"pos -= 0.5 * displacementLen * displacementDirV;":""}

        return pos + displacementLen * (uv0.x * displacementDirU + uv0.y * displacementDirV);
      }
    `),e.space===P.LineMarkerSpace.Screen&&(C.uniforms.add(new m.Matrix4PassUniform("inverseProjectionMatrix",((e,r)=>r.camera.inverseProjectionMatrix))),C.code.add(f.glsl`vec3 inverseProject(vec4 posScreen) {
...
return abs(cos) > 0.001 && t > 0.0;
  ...
  `)),v.addPixelRatio(C,e),a.addCalculateLinearDepth(x),C.code.add(f.glsl`void main(void) {
if (uv0.y == 0.0) {
...
displacedPosScreen.xy = displace(posScreen.xy, prevScreen.xy, screenMarkerSize);`),e.space===P.LineMarkerSpace.Screen&&C.code.add(f.glsl`vec2 displacementDirU = perpendicular(normalizedSegment(posScreen.xy, prevScreen.xy));
...
displacedPosScreen = projectAndScale(pos);`)),C.code.add(f.glsl`
  ...
  `),z&&x.include(l.multipassTerrainTest,e),x.include(o.SliceDraw,e),M.uniforms.add(new h.Float4PassUniform("intrinsicColor",(e=>e.color)),new w.Texture2DPassUniform("tex",(e=>e.markerTexture))),M.include(d.ColorConversion),x.constants.add("texelSize","float",1/r.MARKER_TEXTURE_SIZE),M.code.add(f.glsl`float markerAlpha(vec2 samplePos) {
samplePos += vec2(0.5, -0.5) * texelSize;
float sdf = rgba2float(texture(tex, samplePos)) - 0.5;
float distance = sdf * vSize;
distance -= 0.5;
return clamp(0.5 - distance, 0.0, 1.0);
}`),e.hasTip&&(x.constants.add("relativeMarkerSize","float",r.MARKER_SYMBOL_SIZE/r.MARKER_TEXTURE_SIZE),x.constants.add("relativeTipLineWidth","float",r.MARKER_TIP_THICKNESS_FACTOR),M.code.add(f.glsl`
  ...
  `)),x.constants.add("symbolAlphaCutoff","float",c.symbolAlphaCutoff),M.code.add(f.glsl`
  void main() {
    discardBySlice(vpos);
  ...
  `),x}const z=Object.freeze(Object.defineProperty({__proto__:null,build:x},Symbol.toStringTag,{value:"Module"}));e.LineMarker=z,e.build=x}));
